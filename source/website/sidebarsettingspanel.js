...
class EnvironmentMapPopup extends PopupDialog {
  ...
  ShowPopup(buttonDiv, shadingType, settings, callbacks) {
    ...
    if (shadingType === ShadingType.Phong) {
      ...
      let isPerspective =
        callbacks.getProjectionMode() === ProjectionMode.Perspective;
      if (isPerspective) {
...
class SettingsModelDisplaySection extends SettingsSection {
  ...
  Init(callbacks) {
    ...
    let defaultBackgroundColor =
      '#' + RGBAColorToHexString(this.settings.backgroundColor);
    ...
    this.environmentMapPhongDiv = AddDiv(
      this.contentDiv,
      'ov_sidebar_parameter'
    );
    this.environmentMapPhongInput = AddDiv(
      this.environmentMapPhongDiv,
    ...
    this.environmentMapPbrInput = AddDiv(
      this.environmentMapPbrDiv,
      'ov_sidebar_image_picker'
    );
    ...
    this.environmentMapPbrInput.addEventListener('click', () => {
      this.environmentMapPopup = new EnvironmentMapPopup();
      this.environmentMapPopup.ShowPopup(
        this.environmentMapPbrInput,
        ShadingType.Physical,
        this.settings,
        {
          getProjectionMode: () => {
            return this.callbacks.getProjectionMode();
    ...
    let defaultEdgeColor =
      '#' + RGBColorToHexString(this.settings.edgeSettings.edgeColor);
    ...
    this.thresholdSliderValue.innerHTML =
      this.settings.edgeSettings.edgeThreshold;
  ...
  UpdateEnvironmentMap() {
    ...
    if (this.environmentMapPhongDiv !== null) {
      if (this.settings.backgroundIsEnvMap) {
        UpdateImage(
          this.environmentMapPhongInput,
          this.settings.environmentMapName
        );
        this.environmentMapPhongInput.classList.remove(
          'ov_environment_map_preview_no_color'
        );
      } else {
        this.environmentMapPhongInput.style.backgroundImage = null;
        this.environmentMapPhongInput.classList.add(
          'ov_environment_map_preview_no_color'
        );
      }
    }
  ...
  Update() {
    ...
    if (this.edgeDisplayToggle !== null) {
      ...
      ShowDomElement(
        this.edgeSettingsDiv,
        this.settings.edgeSettings.showEdges
      );

      this.edgeColorPicker.setColor(
        '#' + RGBColorToHexString(this.settings.edgeSettings.edgeColor)
      );
      this.thresholdSlider.value = this.settings.edgeSettings.edgeThreshold;
      this.thresholdSliderValue.innerHTML =
  ...
  UpdateVisibility() {
    let isPhysicallyBased =
      this.callbacks.getShadingType() === ShadingType.Physical;
    if (this.environmentMapPhongDiv !== null) {
      let isPerspective =
        this.callbacks.getProjectionMode() === ProjectionMode.Perspective;
      ShowDomElement(
        this.environmentMapPhongDiv,
        !isPhysicallyBased && isPerspective
      );
    }
    if (this.environmentMapPbrDiv !== null) {
...
class SettingsImportParametersSection extends SettingsSection {
  ...
  Init(callbacks) {
    ...
    this.defaultColorPicker = AddDefaultColorPicker(
      this.defaultColorPickerDiv,
      'Default Color',
    ...
    this.defaultLineColorPicker = AddDefaultColorPicker(
      this.defaultLineColorPickerDiv,
      'Default Line Color',
      this.settings.defaultLineColor,
      (r, g, b, a) => {
  ...
  Update() {
    if (this.defaultColorPicker !== null) {
      this.defaultColorPicker.setColor(
        '#' + RGBColorToHexString(this.settings.defaultColor)
      );
    }
    if (this.defaultLineColorPicker !== null) {
      this.defaultLineColorPicker.setColor(
        '#' + RGBColorToHexString(this.settings.defaultLineColor)
      );
    }
  }
  ...
  UpdateVisibility() {
    if (this.contentDiv !== null) {
      ...
      if (defaultMaterials.length === 0) {
        ...
        ShowDomElement(
          this.defaultColorPickerDiv,
          sources.has(MaterialSource.DefaultFace)
        );
        ShowDomElement(
          this.defaultLineColorPickerDiv,
          sources.has(MaterialSource.DefaultLine)
        );
      }
    }
  }
